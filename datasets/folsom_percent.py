...
class FolsomPercentDataset(Dataset):
    """
    PyTorch Dataset for loading Folsom sky images.
    
    Splits data by percentage: train_percent for training, rest for testing.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            split: "train" or "test"
            train_percent: Percentage of data to use for training (0.0 to 1.0)
                          First train_percent chronologically for training, rest for testing
            image_extensions: Tuple of valid image file extensions
            sample_num: Number of samples to use (for train split, takes first N chronologically)
        ...
        if not 0.0 < train_percent < 1.0:
            raise ValueError(f"train_percent must be between 0.0 and 1.0, got {train_percent}")
        
        # Load all years (2014, 2015, 2016)
        ...
        if irradiance_csv_path.exists():
            ...
            print(f"Loaded {len(self.irradiance_df)} irradiance feature records (years: {year_filter})")
        else:
        ...
        if target_csv_path.exists():
            ...
            print(f"Loaded {len(self.target_df)} target records (years: {year_filter})")
        else:
        ...
        # Get all available keys (that exist in both irradiance and target dicts)
        all_keys = [key for key in self.irradiance_dict.keys() if key in self.target_dict]
        print(f"Total available samples: {len(all_keys)}")
        
        # Sort keys chronologically by timestamp (first percentage for training)
        all_keys.sort()  # Sort by timestamp string (YYYYMMDD_HHMMSS format)
        
        # Split keys chronologically: first train_percent for training, rest for testing
        split_idx = int(len(all_keys) * train_percent)
        train_keys = all_keys[:split_idx]
        test_keys = all_keys[split_idx:]
        
        print(f"Train samples: {len(train_keys)} ({len(train_keys)/len(all_keys)*100:.1f}%) - chronologically first")
        print(f"Test samples: {len(test_keys)} ({len(test_keys)/len(all_keys)*100:.1f}%) - chronologically last")
        
        # Select keys based on split type
        if split == "test":
            self.selected_keys = test_keys
            print(f"Test set: Using {len(self.selected_keys)} samples")
        else:
            # For train split, optionally limit to sample_num (take first N chronologically)
            if sample_num > 0 and len(train_keys) > sample_num:
                self.selected_keys = train_keys[:sample_num]
                print(f"Train set: Using first {len(self.selected_keys)} samples from {len(train_keys)} available (chronological)")
            else:
                self.selected_keys = train_keys
                print(f"Train set: Using all {len(self.selected_keys)} available samples")
